...
{          
  button_error("You have to be admin to edit profiles !", 400);
  button_back();
  closepage();
  exit;
}
...
else if(isset($args['editpasswd']))
{
  ...
  $table->db->RequestGenericFilter("id", $args['id']);
  $table->db->RequestCustom(1);
  if(!$table->db->Query()) {
  ...
  if(!$ret)
  {
    ...
    else
    {
         ...
         )
      {
        ...
        $new_name = strtolower(md5($u->Getpseudo().$u->GetId()) .".".$imagetypes[$picprop[2]]);
...
button("<a href=\"edit_profile.php?id=".$args['id']."\">Return to profile</a>", 300);
}


else if(isset($args['editinfos']))
{
  $localisation = CleanContent($args['localisation']);
  $web = CleanContent($args['web']);
  $speech = CleanContent($args['speech']);
...
else
{
  ...
  if (empty($args['id']))
  {
    button_error("URL error: no id", 400);
    button_back();
    exit;
  }
...
?>
 ...
 <form class="nvform" id="password" name="password" action="edit_profile.php?editpasswd&id=<?php echo $args['id']?>" method="post">
 <table><tr>
 <th colspan="2" align="center"> Password change </th></tr>
 <tr>
 <td width="200px"><label for="pseudo">name : </label></td>
 <td><input type="text" id="pseudo" name="pseudo" size="30" value="<?php echo $u->GetPseudo()?>" readonly /></td>
 </tr><tr>
 ...
 <form class="nvform" action="edit_profile.php?editinfos&id=<?php echo $args['id'] ?>" method="post">
 <table><tr>
 <th colspan="2" align="center"> Personals </th></tr>
 <tr>
 <td width="200px"><label for="localisation">Localisation : </label></td>
 <td><input type="text" id="localisation" name="localisation" size="35" value="<?php echo $u->GetLocalisation()?>" /></td>
 </tr><tr>
 <td><label for="web">Personnal site : </label></td>
 <td><input type="text" id="web" name="web" size="35" value="<?php echo $u->GetWeb()?>" /></td>
 </tr><tr>
 <td><label for="speech">Personnal quote : </label></td>
 <td><textarea id="speech" name="speech" rows="4" cols="35" style="width:250px;"><?php echo $u->GetSpeech() ?></textarea></td>
 </tr><tr>
 <td colspan="2"><center><input type="submit" value="Save" /></center></td>
 </tr></table>
 </form>
 ...
 <!-- Options -->
 <div class="nvform" style="width: 500px;">
 <form class="nvform" id="options" name="options" action="edit_profile.php?editopts&id=<?php echo $args['id']?>" method="post">
 <table><tr>
 ...
 <td><input type="text" id="limit" name="limit" size="3" value="<?php echo $u->GetLimit()?>" /></td>
 </tr><tr>
 <td><label for="sidebar_comments">Sidebar last comments : </label></td>
 <td><input type="text" id="sidebar_comments" name="sidebar_comments" size="3" value="<?php echo $u->GetSidebarComments()?>" /></td>
 </tr><tr>
 <td><label for="sidebar_comlength">Sidebar comments preview length : </label></td>
 <td><input type="text" id="sidebar_comlength" name="sidebar_comlength" size="3" value="<?php echo $u->GetSidebarComLength()?>" /></td>
 </tr><tr>
 <td colspan="2"><center><input type="submit" value="Save" /></center></td>
 </tr></table>
 </form>

 <script type="text/javascript">update_profile_optionform_fields("<?php echo $u->GetSort() ?>")</script>
 </div>

  <!-- Avatar -->
 <div class="nvform" id="avatar" name="avatar" style="width: 500px;">
 <form enctype="multipart/form-data" class="nvform" action="edit_profile.php?upavatar&id=<?php echo $args['id']?>" method="post">
 <table><tr>
